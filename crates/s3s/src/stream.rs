...
pin_project_lite::pin_project! {
    #[derive(Debug, Clone)]
    pub(crate) struct BodyWrapper<B>{
        #[pin]
        inner : http_body_util::BodyStream<B>
    }
}
...
impl<B> BodyWrapper<B> {
    pub fn new(inner: B) -> Self {
        Self {
            inner: http_body_util::BodyStream::new(inner),
        }
    }
}

impl<B> Stream for BodyWrapper<B>
where
    B: http_body::Body,
    B::Data: Into<Bytes>,
        ...
        Poll::Ready(loop {
            match std::task::ready!(this.inner.as_mut().poll_next(cx)?) {
                Some(frame) if frame.is_data() => {
                    let Ok(data) = frame.into_data() else { panic!("already checked") };
                    break Some(Ok(data.into()));
                }
                Some(_) => continue,
                None => break None,
            }
...
impl<B: http_body::Body> ByteStream for BodyWrapper<B>
where
    B::Data: Into<Bytes>,
    StdError: From<B::Error>,
{
    fn remaining_length(&self) -> RemainingLength {
        http_body::Body::size_hint(&self.inner).into()
    }
}
